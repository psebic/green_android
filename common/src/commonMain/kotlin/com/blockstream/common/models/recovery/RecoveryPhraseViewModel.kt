    ...
    init {
        viewModelScope.launch {
            ...
            session.ifConnectedSuspend {
                val credentials = providedCredentials ?: (if (isLightning) Credentials(mnemonic = session.deriveLightningMnemonic()) else session.getCredentials())

                mnemonic.value = credentials.mnemonic ?: ""
                mnemonicWords.value = credentials.mnemonic?.split(" ") ?: listOf()
